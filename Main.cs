...
namespace LazyCSharp {
	...
	public class LazyList<T> : IEnumerable<T>
	{
		...
		public List<T> ToRegularList(ulong count, ulong start = 0)
		{
			...
			ForEach(component => result.Add(component.Head.Value), count, start);
			return result;
		}
		
		public void ForEach(Action<LazyListComponent<T>> action, ulong count, ulong start = 0)
		{
		...
		/// <param name="state">The initial s value used to generate the first element of the sequence. Will be used recursevly.</param>
			...
			Func<TState, (LazyType<TValue> head, TState next)> step, Func<TState, bool>? canContinue = null)
		{
			return new LazyList<TValue>(() =>
			{
				...
				LazyList<TValue> tail = Range<TState, TValue>(next, step, canContinue);
				...
				tail = canContinue?.Invoke(state) == false ? new LazyList<TValue>(() => null) : tail;
		...
		public readonly static LazyList<ulong> InfiniteInt_Save =		Range<ulong, ulong>(0, n => (new LazyType<ulong>(() => n), next: n + 1), s => s < ulong.MaxValue);
		public readonly static LazyList<ulong> InfiniteEven_Save =		Range<ulong, ulong>(0, n => (new LazyType<ulong>(() => n), next: n + 2), s => s < ulong.MaxValue - 1);
		public readonly static LazyList<ulong> InfiniteOdd_Save =		Range<ulong, ulong>(1, n => (new LazyType<ulong>(() => n), next: n + 2), s => s < ulong.MaxValue);
		public readonly static LazyList<ulong> InfiniteFibonacci_Save =	Range<(ulong a, ulong b), ulong>((0, 1), state => (head: new LazyType<ulong>(() => state.b), next: (state.b, state.a + state.b)), s => ulong.MaxValue - s.a >= s.b) ;
		//0x000FFFFFFFFFFFFF
		/// <summary>
		/// A list of elements calculated by 1/n for n = 1, 2, 3,...
		/// </summary>
		public static LazyList<double> InfiniteHarmonic_UnSafe = Range<int, double>(1, n => (new LazyType<double>(() => 1.0 / n), next: n + 1)); // smallest Value: 0x000FFFFFFFFFFFFF
		/// <summary>
		...
		public static LazyList<int> InfinitePrimes_Unsafe = Range<List<int>, int>(
		[2], n =>
		{
			return (new(() =>
			{
		...
		public static LazyList<ulong> InfinitePowerOf10_Save = Range<ulong, ulong>(0, n => (new LazyType<ulong>(() => (ulong)Math.Pow(10, n)), n + 1), s => max10 > Math.Pow(10, s));
		public static LazyList<ulong> InfinitePowerOf2_Save = Range<ulong, ulong>(0, n => (new LazyType<ulong>(() => (ulong)Math.Pow(2, n)), n + 1), s => max2 > Math.Pow(2, s));
